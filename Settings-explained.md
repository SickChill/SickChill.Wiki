...
As you can see below SickChill supports a massive amount of customization settings, and might lead to some confusion. With this wiki we will try to explain the settings in more details. As many of those settings are for advanced user only, make sure you understand what the settings does before setting.
...
`SR Version:`  
This show the exact version of SickChill you are using. SickChill gets updated with so called Commits (code updates). And by knowing the last Commit your particulair SickChill is running, you can tell if you missed updates and/or helps the Develpors trouble shooting a problem you have.  
`SR User:`  
...
`SR Database file:`  
Shows the location of your sickbeard.db file. It includes all your shows for SickChill.  
`SR Cache Dir:`  
...
`Python Version:`  
This shows your Python version. SickChill needs a minimal version of 2.7 but the more recent the better. Also make sure pyopenssl and cryptography modules are included in your Python version.  
...
You can manually set a time at witch SickChill updates/refreshes the shows information from the indexers.  9airdates etc.) Best to use a time at night when the device is idle.  
`Update shows on startup`  
Feature was removed  
`Update shows on snatch`  
Lers SickChill update the shows data when an episode is snatched.  
...
`HTTP username`  
You can set a username and pasword when opening the SickChill website.  
`HTTP password`  
You can set a username and pasword when opening the SickChill website.  
`HTTP port`  
...
`Branch version:`  
This shows the SickChill versions you can select. Master branch is the default and the Develop branch is hidden, and can only be access if a GitHub account is added. However end-users should always use Master and only switch to Develop if asked.  
`GitHub username`  
...
* This sections allows you to create an backup of your SickChill settings. The files include :  
...
`Usenet retention`  
If you use NZB's than check what the retention of your Usenet server is and add the value in this field. This prevents SickChill from sending episode NZB's that are not on your Usenet server anymore. Most payed Usenet servers have a retention above 1000 days, but ISP servers generally offer much less like 31 days.  
`Ignore words`  
If SickChill comes across any of those words in the title of a search result, the result will be SKIPPED. Apply's to all shows. USE WITH CAUTION!   
`Require words`  
SickChill SKIPPES/IGNORES all search results that dont have those words in the title. Apply's to all shows. USE WITH CAUTION!  
`Allow high priority`  
...
## Post Processing  
* In this section you can use post process options that SickChill should preform after a episode is downloaded. For example renaming of the file etc. More information can also be found under [Post Processing](https://github.com/SickChill/SickChill/wiki/Post-Processing)  

![postprocessing-1](https://cloud.githubusercontent.com/assets/7928052/13013585/5c1a412a-d1b0-11e5-8d3c-804ff72ffbdb.png)

`TV Download Dir`  This is the folder that SickChill monitors for newly downloaded files/episodes. Once a new file is found the post-processing starts to move the file to your shows folder and take any other actions that you have configured.   
...
Should SickChill move any files that are downloaded with the episode file like external subtitles etc.?  
...
Lets you select if SickChill should remove or delete empty folders left after moving the episode file.  
...
`Unpack`  
In case a rar collection is detected in the TV Download Dir, SickChill can unrar them.   
`Use Failed Downloads`  
...
`Delete Failed`  
In case a failed download is detected in the TV Download Dir, SickChill will delete the folder.  
